import { dbConfig, Allocation, Merchant, AllocationEvent, AllocationEventOffer, Wine } from "../../common/models"
...
export class AllocationDaos {
    ...
    async listAllocationLastPurchases(cellarId:number, limit: number = 25, page: number = 0,  filter:IFilter){
        ...
        const query:string = 'SELECT "AllocationEvents"."allocationId", "Collections"."allocationEventId", "Collections"."vintageId", MAX("CollectionEvents"."actionDate") AS "lastPurchased" FROM "Collections" INNER JOIN "CollectionEvents" on "Collections"."id" = "CollectionEvents"."collectionId"  INNER JOIN "AllocationEvents" on "AllocationEvents"."id" = "Collections"."allocationEventId" WHERE "CollectionEvents"."action" = \'PurchasedOn\' AND "Collections"."cellarId" =  :userId  GROUP BY "AllocationEvents"."allocationId", "Collections"."allocationEventId", "Collections"."vintageId"'
    ...
    async getAllocationById(allocationId: string) {
        return Allocation.findOne({
            ...
            include: [
                {model: Merchant, as:"merchant", attributes:['id','name','userId','producerId']},
                {model: AllocationEvent, as:"events", include:[{
                    model: AllocationEventOffer, include:[{
                        model:Wine, as:"wine"
                    }]
                }]}]
        });
